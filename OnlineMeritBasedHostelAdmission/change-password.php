...
$udate=date('d-m-Y h:i:s', time());;
	...
	if($row_cnt>0)
	{
		$con="update user_reg set password=?,passUdateDate=?  where stid=?";
...
$ai=$_SESSION['id'];
	...
	if($row_cnt>0)
	{
		$con="update user_reg set password=?  where stid=?";
$chngpwd1 = $mysqli->prepare($con);
...
<?php /*/$result ="SELECT passUdateDate FROM user_reg WHERE stid=?";
