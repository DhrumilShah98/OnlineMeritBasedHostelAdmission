...
// if male
  if($row['gender'] == 'Male')
  {
        if($row['caid'] ==1)
        {
          ...
          $total_seats_retrieval = mysqli_query( $mysqli,"select seats FROM hostel_seats where hid='".$row['hid']."'");
          $total_seats_no = mysqli_fetch_row($total_seats_retrieval);
          echo " seats available for total seats   is: {$total_seats_no[0]} <br>";
                      ...
                      if($seats_no[0] <=0)
                      {
                        echo "All seats reserved General Male <br>";
                              ...
                              $stmt->execute();
                               ...
                               // decrementing the seat value
                              $total_seats_no[0] = $total_seats_no[0] -1;
                              $query="Update hostel_seats Set seats=".$total_seats_no[0]." where hid='".$row['hid']."'";
                              $stmt = $mysqli->prepare($query);
                              $stmt->execute();
